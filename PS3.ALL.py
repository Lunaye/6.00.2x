...
class Patient(object):
    ...
    def update(self):
        ...
        for r in self.viruses:
            try:                
                child = r.reproduce(popDensity)
...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    for i in range(numTrials):
        ...
        for i in range(300):
            pop[i].append(patient.update())

    population = [i/numTrials for i in pop]

    pylab.plot(population, label = 'Virus Population')
...
class ResistantVirus(SimpleVirus):
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        NoChildException if this virus particle does not reproduce.
        """
        if (all(self.isResistantTo(d) for d in activeDrugs) and random.random() <= self.getMaxBirthProb() * (1 - popDensity)):           
            resistances = {}
            for r, v in self.resistances.items():
...
class TreatedPatient(Patient):
    ...
    def update(self):
        ...
        for i in self.viruses:
            try:                
                self.viruses.append(i.reproduce(popDensity, self.getPrescriptions()))
            except NoChildException:
                pass
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    pylab.plot(resisv, label = 'Resistant Virus Population')
    pylab.title('Simulation Without Drug')
    pylab.xlabel('Timestep')
    pylab.ylabel('Virus Population')
    pylab.legend(loc='best')
